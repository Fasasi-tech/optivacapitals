            ...
            <Table>
                ...
                {paginatedData.map((i, index) =>(
                    <TableRow key={index}>
                        ...
                        <TableCell ><p className={`p-1 w-20 text-center rounded-lg text-white font-semibold ${i.Status === 'Pending Approval' ? 'bg-blue-600' : i.Status === 'New' ? 'bg-yellow-300' : i.Status === 'Approved' ? 'bg-green-400' : i.Status === 'Rejected' ? 'bg-red-600' :'bg-teal-600'}`  }>{i.Status}</p></TableCell>
                    </TableRow>
                    ))}
                </TableBody>
            </Table>
